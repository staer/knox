...
Client.prototype.putMultipartFile = function(src, filename, headers, fn) {
    ...
    // We return a new event emitter to return various states of the multipart upload
    //  'uploadInit' - sent once initialization of the multpart upload is complete returns: uploadId, numParts, partSize
    //  'partComplete' - sent after a part has been uploaded, returns: partNum, ETag, bytesUploaded
    //  'uploadComplete' - sent when the upload has completed
    var emitter = new events.EventEmitter;
    
    // temeporary abortUpload that does nothing, will be replaced once an uploadId is acquired.
    // Prevents users from getting an instant error if they call abort right after they initiate the upload
    emitter.abortUpload = function() {
        console.log('Upload has not started yet, nothing to abort!');
    };
        ...
        if(filesize < MINIMUM_SIZE) {
            ...
            fs.open(src, 'r', function(err, fd) {
                ...
                var finishUpload = utils.after(num_chunks, function(uploadId, eTagRegistry){
                    ...
                    req.on('response', function(res) {
                        // if we got anything but a 200, consider it an error and sent it back as such
                        return fn(null, res);
                    });
                    req.end(buff);                    
                });
                
                // A close file method wrapped with after. This method will only be executed
                // after it has been called "num_chunks" times. THis ensures that we don't prematurely
                // close the file while we are still trying to read it.
                var closeFile = utils.after(num_chunks, function(){
                ...
                req.on('response', function(res){
                    ...
                    // if we got anything but a 200:
                    // stop the upload and execute the callback function
                    ...
                    res.on('end', function() {
                        ...
                        // Now that we have a valid uploadId, we can write the abort code
                        emitter.abortUpload = function() {                            
                            var resource = filename + "?uploadId=" + uploadId;
                            var abortRequest = self.request("DELETE", resource);
                            abortRequest.on('response', function(res) {
                                // What to do once we've aborted?
                            }).end();
    
                            // TODO: Write abort upload code!
                        };
                        emitter.emit('uploadInit', uploadId, num_chunks, chunk_size, filesize);
                            ...
                            fs.read(fd, buff, 0, chunk_size, (i-1)*chunk_size, function(err, bytesRead, buffer) {
                                ...
                                uploadRequest.on('response', function(response) {
                                    if(response.statusCode != 200) {
                                        // TODO, handle part upload retries?
                                        // Emit an 'error' event so that we can abort upload if necessary?
                                    } else {
                                        ...
                                        emitter.emit('partComplete', uploadId, partNum, response.headers.etag);
                                        
                                        // finishUpload() will only be executed after being called num_chunks times.
                                        // It was wrapped using the utils.after function
